...
SECOND_TEST="tests/test_belex_add_u16.py"

for i in {1..1}
do
    echo "${COMMAND} ${PLATFORM} ${OPTIMIZATIONS} ${SPECIFIC_TEST} 2>&1 | tee ${OUTFILE_PREFIX}_${i}.txt | ${FILTER}"

    ${COMMAND} ${PLATFORM} ${OPTIMIZATIONS} ${SPECIFIC_TEST} 2>&1 | tee ${OUTFILE_PREFIX}_BASIC_ADD.txt | ${FILTER}
    ${COMMAND} ${PLATFORM} ${OPTIMIZATIONS} ${SECOND_TEST} 2>&1 | tee ${OUTFILE_PREFIX}_BELEX_ADD.txt | ${FILTER}
